---
title: "Evaluating Effects of Population Demographics on Access to Sexual Health Care: A Toronto Case Study"
#subtitle: "A subtitle"
author:
- name: Audreana Rossi
  student_number: 400325255
- name: Moira Chantelle Lobo
  student_number: 400242471
- name: Eva Boomsma
  student_number: 400325609
- name: Heléna Muirhead-Hunt
  student_number: 400170333
- name: Eva Novoselac
  student_number: 400314401
- name: Samantha Kirtz
  student_number: 400332730
- name: Peri Juskiw
  student_number: 400264473
- name: Samantha Victoria Baginski
  student_number: 400268593
subject: "ENVSOCTY 4GA3: Applied Spatial Statistics"
abstract: "This report examines the accessibility to sexual health clinics operated by the City of Toronto, and the correlation of this variable with the spatial distribution of various disadvantaged populations."
keywords: "Accessibility, Sexual Health Clinics, Vulnerable Populations, Spatial Statistics,
Spatial Analysis"
date: "4/19/2024"
output:
  pdf_document:
...
library(spaa) # SPecies Association Analysis
library(spatstat) # Spatial Point Pattern Analysis, Model-Fitting, Simulation, Tests
library(spdep) # Spatial Dependence: Weighting Schemes, Statistics # Spatial Dependence: Weighting Schemes, Statistics
...
toronto_das <- left_join(toronto_das,
                        acc_sex, 
                        by = "DAUID") # DAs must be listed first to retain its geometry features
...
```{r echo=FALSE}
toronto_das_clean <- toronto_das |>
  filter(!is.na(acc_sex),
         !is.na(Pop15plus))

toronto_das_clean.nb <- poly2nb(pl = toronto_das_clean, queen = TRUE)
toronto_das_clean.w <- nb2listw(toronto_das_clean.nb)
moran.test(toronto_das_clean$acc_sex, toronto_das_clean.w)
moran.plot(toronto_das_clean$acc_sex, toronto_das_clean.w)

accessmp <- moran.plot(toronto_das_clean$acc_sex, 
                       toronto_das_clean.w, 
                       xlab = "Clinic accessibility by DA", 
                       ylab = "Spatially lagged clinic accessibility by DA")
...
plot_1 <- ggplot() +
  geom_sf(data = toronto_das,
          aes(fill = Median_Income), color = NA) + 
...
plot_2 <- ggplot() +
  geom_sf(data = toronto_das, 
          aes(fill = Indigenous_Pop), color = NA) + 
  geom_sf(data = clinics) + # Plot the clinics data
...
plot_3 <- ggplot() +
  geom_sf(data = toronto_das, 
          aes(fill = One_Parent_Families), color = NA) + 
  geom_sf(data = clinics) + # Plot the clinics data
  scale_fill_distiller(palette = "Purples", direction = 1) +
  labs(fill = "One parent families by DA") + 
  theme_minimal() +
  theme(legend.position = "bottom")
...
plot_4 <- ggplot() +
  geom_sf(data = toronto_das, 
          aes(fill = Visible_Minority_Pop), color = NA) + 
  geom_sf(data = clinics) + # Plot the clinics data
...
gridExtra::grid.arrange(plot_1, plot_2, plot_3, plot_4, ncol = 2)
```
...
ggplot() +
  geom_sf(data = toronto_das, 
          aes(fill = acc_sex), 
          color = NA) + # Set fill color based on total_impedance and remove outline
  geom_sf(data = clinics) + # Plot the clinics data
...
```{r, echo=FALSE}
### Local Moran's I map
lmp.pop15plus <- localmoran.map(toronto_das_clean, 
                                toronto_das_clean.w, 
                                VAR = "Pop15plus", 
                                by = "DAUID")
...
file = "figures/lmp.pop15plus.html")
webshot2::webshot(url = "figures/lmp.pop15plus.html", 
                  file = "figures/lmp.pop15plus.png",
                  delay=1,
                  zoom=4,
                  vheight=500)
```
...
cor.test(toronto_das$Median_Income, 
         toronto_das$Visible_Minority_Pop)
cor.test(toronto_das$Median_Income, 
         toronto_das$Indigenous_Pop)
cor.test(toronto_das$Median_Income, 
         toronto_das$One_Parent_Families)
cor.test(toronto_das$Indigenous_Pop,
         toronto_das$Visible_Minority_Pop)
cor.test(toronto_das$One_Parent_Families, 
         toronto_das$Visible_Minority_Pop)
cor.test(toronto_das$One_Parent_Families, 
         toronto_das$Indigenous_Pop)
...
toronto_das_clean <- toronto_das |>
  ...
  filter(!is.na(Median_Income),
         !is.na(Visible_Minority_Pop),
         !is.na(One_Parent_Families),
         !is.na(acc_sex))

predictormodel <- lm(formula = acc_sex ~ Median_Income + Visible_Minority_Pop + One_Parent_Families, 
                     data = toronto_das_clean |> st_drop_geometry())
...
toronto_das_clean$predictormodel.e <- predictormodel$residuals

#now we must merge regression clean to have geometry data, with DAs
#residualmap <- merge(toronto_das_clean, regressionclean, by = "DAUID")

toronto_das_clean |>
  mutate(residual = ifelse(predictormodel.e <=0, "Negative", "Positive")) |>
ggplot() +
...
regressionclean.w <- toronto_das_clean |> 
  poly2nb() |>
  nb2listw() 

moran.test(toronto_das_clean$predictormodel.e, 
           regressionclean.w)
```
